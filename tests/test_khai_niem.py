...
def test_nhan_khai_niem_ngang_chi_ghep():
    a = k.label_concept(_anh(ngang=True, dung=["ghép dọc với một ảnh ngang cùng tone", "cat_ngang: true"]))
    assert a["dung"] == ["ghép dọc với một ảnh ngang cùng tone"]


def test_nhan_khai_niem_chart_hoac_mat_thi_bo():
    assert k.label_concept(_anh(loai="chart"))["dung"] == []
    a = k.label_concept(_anh(mat=2))
    assert a["dung"] == [] and a["ghi_chu"][0].startswith("❌")


def test_nhan_khai_niem_giu_nguyen_khi_vision_da_loai():
    a = k.label_concept(_anh(lien_quan=False, dung=[], ghi_chu=["❌ KHÔNG LIÊN QUAN BÀI (vision) → KHÔNG DÙNG"]))
