...
def test_viec_loc_theo_moc_thoi_gian():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            assert [v["id"] for v in ha.job(tu_ts=200)] == ["t2"]
        finally:
            _go()
...
def test_trang_thai_va_dem_dang_chay():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            assert ha.status("t2") == "running"
            assert ha.status("khong-co") == "", "task khong co phai la '' "
            assert ha.count_form_run() == 2, "chi ready+running moi tinh"
            assert ha.count_form_run(tru_tid="t1") == 1, "phai tru chinh no ra"
        finally:
            _go()
...
def test_metadata_hong_thi_thanh_dict_rong_khong_nem():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            assert ha.last_run("t1")["metadata"] == {}
        finally:
            _go()
...
def test_lan_chay_cuoi_task_chua_chay_la_dict_rong():
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            assert ha.last_run("t1") == {}, "chua chay != khong doc duoc"
        finally:
            _go()
...
def test_thieu_kanban_db_thi_tra_None_khong_phai_rong():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        try:
            assert ha.job() is None
            assert ha.one_job("t1") is None
            assert ha.status("t1") is None
            assert ha.count_form_run() is None
            assert ha.last_run("t1") is None
            assert ha.last_run_many(["t1"]) is None
        finally:
            _go()
...
def test_sai_schema_cung_tra_None():
    ...
    with tempfile.TemporaryDirectory() as tmp:
        ...
        ha.kanban_db = lambda: p
        try:
            assert ha.job() is None, "doi schema ma van tra rong la hong cam"
        finally:
            _go()
...
def _tao_task(ket_qua, **kw):
    ...
    subprocess.run = _run
    try:
        return ha.create_task("Anh: Tin X", "dre", "than task", **kw), da_chay
    finally:
        subprocess.run = cu
