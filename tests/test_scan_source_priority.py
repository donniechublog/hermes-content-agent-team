...
def test_same_model_named_two_ways_counts_as_one():
    ...
    _, con_lai, _ = scan_models.prefer_arena([], climbs)
    assert [c["board"] for c in con_lai] == ["webdev"], "ban AA phai nhuong cho arena"
    # Gach noi trong ten CO chu hoa (`GLM-5.3`) khong duoc lam lech phep so.
    hai = [_climb("webdev", "glm-5.3"), _climb("coding", "GLM-5.3")]
    _, con2, _ = scan_models.prefer_arena([], hai)
    assert [c["board"] for c in con2] == ["webdev"]
...
def test_a_shorter_version_is_not_the_same_model():
    ...
    _, con_lai, _ = scan_models.prefer_arena([], climbs)
    assert {c["board"] for c in con_lai} == {"webdev", "intelligence"}
...
def test_independent_benchmarks_are_untouched():
    ...
    _, con_lai, _ = scan_models.prefer_arena([], climbs)
    assert {c["board"] for c in con_lai} == {"webdev", "livebench", "tbench"}, \
...
def test_release_yields_only_to_a_first_sighting_on_arena():
    ...
    moi = [_climb("webdev", "claude-opus-5-5-max", previous_rank=None)]
    leo = [_climb("webdev", "claude-opus-5-5-max", previous_rank=3)]
    con, _, bo = scan_models.prefer_arena([_release("Claude Opus 5.5")], moi)
    assert con == [] and [r["original_name"] for r in bo] == ["Claude Opus 5.5"]
    con2, _, bo2 = scan_models.prefer_arena([_release("Claude Opus 5.5")], leo)
    assert [r["original_name"] for r in con2] == ["Claude Opus 5.5"] and bo2 == []
...
def test_it_says_what_it_dropped():
    ...
    scan_models.prefer_arena([], [_climb("webdev", "glm-5.3"), _climb("coding", "GLM-5.3")],
                             in_log=dong.append)
    assert dong and "coding|GLM-5.3" in dong[0]
