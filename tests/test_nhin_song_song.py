...
import chuan_bi.nhin as nhin                                  # noqa: E202,E402
...
def test_429_duoc_thu_lai_roi_thanh_cong():
    ...
    try:
        r = nhin._goi_router(object(), _ngu=ngu.append)
    finally:
...
def test_401_khong_thu_lai():
    ...
    try:
        try:
            nhin._goi_router(object(), _ngu=lambda s: None)
...
def test_het_luot_thu_thi_nem_429():
    ...
    try:
        try:
            ...
            assert e.code == 429 and len(goi) == len(nhin._CHO_THU_LAI) + 1, len(goi)
            return
    finally:
        urllib.request.urlopen = cu
...
def test_mot_anh_hong_khong_giet_ca_lo():
    with tempfile.TemporaryDirectory() as t:
        ...
        ra, dung_duoc, chua_nhin = nhin._nhin_anh(anh, {}, "", wd)
