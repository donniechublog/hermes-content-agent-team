#!/usr/bin/env python3
"""`md_bright_html` dựng đúng bốn cú pháp mà 9router sinh ra (issue D2).

Bo `markdown` khoi requirements duoc vi dau vao KHONG phai markdown bat ky:
chinh `monitor_9router.py` sinh ra tep .md do, nen tap cu phap dong — `#`/`##`,
...
def test_khong_con_phu_thuoc_goi_markdown():
    ...
    # `import markdown`.
    import ast
    cay = ast.parse((ROOT / "journal_web.py").read_text(encoding="utf-8"))
