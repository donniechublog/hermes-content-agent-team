...
def test_ROOT_cua_goi_la_goc_du_an():
    ...
    assert chung.ROOT == GOC, \
        f"chuan_bi.chung.ROOT tro sai: {chung.ROOT} (phai la {GOC})"


def test_ROOT_khop_env_load():
    """env_load.ROOT la ban goc su that; hai cho lech nhau la mot cho sai."""
    assert chung.ROOT == Path(env_load.ROOT).resolve(), \
        f"{chung.ROOT} != env_load.ROOT {env_load.ROOT}"
...
def test_mat_tien_anh_chuan_bi_tro_cung_cho():
    """Vai/test goi qua `cb.DRAFTS`; no chi la tai xuat cua chung.DRAFTS nen hai
    ben lech nhau la co mot ban sao thu hai dang song."""
    assert cb.ROOT == chung.ROOT and cb.DRAFTS == chung.DRAFTS, (cb.DRAFTS, chung.DRAFTS)
