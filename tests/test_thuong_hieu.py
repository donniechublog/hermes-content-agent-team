#!/usr/bin/env python3
"""Ảnh thương hiệu (anh_thuong_hieu.py) — "tin về Big Brand mà kho ảnh mỏng thì
...
def _h(tieu_de, tom=""):
    return [x["hang"] for x in th.hang_trong_tin(tieu_de, tom)]
...
def test_truy_van_hoi_thang_tru_so_khong_hoi_ten_tran():
    cau = [c for _, c in th.truy_van("qualcomm")]
...
def test_truy_van_dung_ca_ten_phu_va_co_tran():
    ten = [t for t, _ in th.truy_van("meta")]
    assert ten == ["Meta Platforms"] * 3 + ["Facebook"], ten
    assert len(th.truy_van("meta")) <= th.TOI_DA_TRUY_VAN
...
def test_loc_doi_ten_hang_trong_ten_tep():
    ...
    assert [c["alt"] for c in th.loc_commons(pages, "Qualcomm")] \
...
def test_loc_bo_nhieu_rung_amazon_va_qua_tao():
    ...
    assert [c["alt"] for c in th.loc_commons(pages, "Amazon")] == ["Commons: Amazon Spheres Seattle.jpg"]
    tao = {"1": _pg("Apple Park aerial view.jpg"), "2": _pg("Apple tree in blossom.jpg")}
    assert [c["alt"] for c in th.loc_commons(tao, "Apple")] == ["Commons: Apple Park aerial view.jpg"]
...
def test_loc_khong_bat_nham_thang_ba_va_union_square():
    ...
    assert len(th.loc_commons(pages, "Apple")) == 2
...
def test_loc_khong_doi_tu_chung_trong_ten_cong_ty():
    ...
    assert len(th.loc_commons(pages, "Mistral AI")) == 1


def test_loc_bien_gioi_tu_chat_hon_chuoi_con():
    pages = {"1": _pg("Armstrong Building Ohio.jpg"), "2": _pg("Arm Cambridge office.jpg")}
    assert [c["alt"] for c in th.loc_commons(pages, "Arm Holdings")] == ["Commons: Arm Cambridge office.jpg"]
...
def test_loc_jpeg_truoc_png_roi_moi_den_kich_thuoc():
    ...
    assert [c["alt"] for c in th.loc_commons(pages, "Samsung")] == [
...
def test_loc_rong_khi_khong_co_gi():
    assert th.loc_commons({}, "Qualcomm") == [] and th.loc_commons(None, "Qualcomm") == []
...
def test_tep_claim_chi_lay_ten_tep():
    ...
    assert th._tep_claim(cl, "P154") == ["X logo.svg"]


def test_hang_co_model_chi_hang_lam_model():
    """Chỉ hãng có model trên bảng mới đáng mở browser đi chụp bảng."""
    assert th.hang_co_model("deepseek") and th.hang_co_model("openai")
    assert not th.hang_co_model("qualcomm") and not th.hang_co_model("tsmc")
...
def test_cau_hoi_vision_khong_hoi_co_phai_anh_cua_tin():
    ...
    c = th.cau_hoi_vision("Kiện Anthropic", {"hang": "Anthropic", "loai": "nguoi",
                                             "nguoi": "Dario Amodei", "vai": "CEO"})
    assert "Dario Amodei" in c and "CHAN DUNG" in c and "LIEN_QUAN" in c
    c = th.cau_hoi_vision("DeepSeek gọi vốn", {"hang": "DeepSeek", "loai": "logo"})
    assert "THE LOGO" in c and "DeepSeek" in c
    c = th.cau_hoi_vision("Qualcomm ký", {"hang": "Qualcomm", "loai": "anh"})
    assert "BOI CANH" in c and "mit tinh" in c


def test_nhan_chan_dung_doi_khai_dung_ten():
    a = th.nhan_thuong_hieu(_anh(thuong_hieu={"hang": "Anthropic", "loai": "nguoi",
...
def test_nhan_the_logo_go_ghi_chu_chart_mau_thuan():
    ...
    a = th.nhan_thuong_hieu(_anh(thuong_hieu={"hang": "DeepSeek", "loai": "logo", "nen": "tối"},
...
def test_nhan_the_logo_chi_bia_va_noi_ro_nen():
    ...
    b = th.nhan_thuong_hieu(_anh(thuong_hieu={"hang": "Anthropic", "loai": "logo", "nen": "sáng"}))
    assert '"nen": "sang"' in b["ghi_chu"][0]


def test_nhan_bang_xep_hang_noi_ro_khong_phai_bang_cua_tin():
    a = th.nhan_thuong_hieu(_anh(thuong_hieu={"hang": "DeepSeek", "loai": "xep_hang",
...
def test_ua_wikimedia_co_duong_lien_he():
    ...
    assert anh_khai_niem.UA == env_load.UA_WIKI
