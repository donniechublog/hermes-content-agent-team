...
def test_chon_theo_k_dung_so_thu_tu_1_den_n():
    ds = [{"a": 1}, {"a": 2}, {"a": 3}]
    assert mc.pick_by_k(1, ds, "x")[0] == {"a": 1}
    assert mc.pick_by_k(3, ds, "x")[0] == {"a": 3}
    assert mc.pick_by_k("2", ds, "x")[0] == {"a": 2}, "chuoi so cung phai nhan"
...
def test_chon_theo_k_ngoai_dai_thi_bao_ro_dai_hop_le():
    ...
    for k in (0, 3, -1, 99, None, "hai", ""):
        muc, loi = mc.pick_by_k(k, ds, "muc 4")
...
def test_chon_theo_k_khong_bao_gio_lay_phan_tu_cuoi_bang_so_am():
    ...
    ds = [{"a": 1}, {"a": 2}, {"a": 3}]
    assert mc.pick_by_k(-1, ds, "x")[0] is None
...
def test_don_tom_tat_doi_em_dash_thanh_dau_phay():
    ...
    for dau in ("—", "–"):
        tom, canh = mc.single_summary(f"Nvidia mở kho {dau} ai cũng tải được", "muc 1")
...
def test_don_tom_tat_khong_dung_toi_dau_gach_noi_binh_thuong():
    tom, canh = mc.single_summary("GPT-5 Codex Max ra mắt", "muc 1")
    assert tom == "GPT-5 Codex Max ra mắt"
    assert canh == []


def test_don_tom_tat_canh_bao_dai_dung_nguong_15_tu():
    assert mc.single_summary(" ".join(["từ"] * 15), "muc 1")[1] == []
    canh = mc.single_summary(" ".join(["từ"] * 16), "muc 1")[1]
...
def test_gom_muc_cong_diem_tong_tu_ba_thanh_phan():
    ...
    items, loi = mb.gather_item([_p(k=1, score_technical=20, score_relevance=15)], cands)
    assert loi == [], loi
...
def test_gom_muc_chan_vai_nop_trung_mot_tin():
    ...
    items, loi = mb.gather_item([_p(k=1), _p(link="https://a.vn/1"), _p(k=2)], cands)
...
def test_gom_muc_van_nhan_link_cho_tuong_thich():
    ...
    items, _loi = mb.gather_item([_p(link="https://a.vn/1")], cands)
...
def test_gom_muc_diem_khong_phai_so_thanh_0_chu_khong_no():
    import manifest_build as mb
    cands = [_c("https://a.vn/1", "Tin một")]
    items, loi = mb.gather_item([_p(k=1, score_technical="24 diem", score_relevance=None)], cands)
...
def test_gom_muc_category_la_thi_ve_TOOL_va_noi_ro():
    import manifest_build as mb
    cands = [_c("https://a.vn/1", "Tin một")]
    items, loi = mb.gather_item([_p(k=1, category="VUI VE")], cands)
...
def test_gom_muc_nhan_ca_nhan_tieng_viet_lan_tieng_anh():
    ...
    import manifest_build as mb
    cands = [_c("https://a.vn/1", "Tin một"), _c("https://b.vn/2", "Tin hai")]
    items, loi = mb.gather_item([_p(k=1, category="ENGINEERING"), _p(k=2, category="MÔ HÌNH")], cands)
...
def test_gom_muc_don_em_dash_cua_summary():
    import manifest_build as mb
    cands = [_c("https://a.vn/1", "Tin một")]
    items, loi = mb.gather_item([_p(k=1, summary_vi="Mở kho — ai cũng tải")], cands)
...
def test_cat_tran_giu_8_tin_diem_cao_nhat():
    ...
    loi = []
    ra = mb.crop_ceiling(items, set(), loi)
    assert len(ra) == mb.MAX_PICK
...
def test_cat_tran_khong_tinh_muc_BAT_BUOC_vao_tran():
    ...
    import manifest_build as mb
    items = [{"link": f"https://a.vn/{i}", "title": f"T{i}", "score": i} for i in range(1, 13)]
    bb = {required.chuan_link("https://a.vn/1")}
    ra = mb.crop_ceiling(items, bb, [])
    assert len(ra) == mb.MAX_PICK + 1
...
def test_muc_tu_nop_chan_link_khong_phai_URL():
    ...
    for xau in ("blank", "khong co", "a.vn/1", "ftp://a.vn/1", "  "):
        assert mg._item_from_submit({"title": "Tin một", "link": xau}, 1, [], "nova", "nova") is None, xau


def test_muc_tu_nop_thieu_title_thi_bo():
    import manifest_write as mg
    assert mg._item_from_submit({"link": "https://a.vn/1"}, 1, [], "nova", "nova") is None


def test_muc_tu_nop_suy_via_tu_ten_mien():
    """via = NGUON TIN, khong phai kenh phat hien."""
    import manifest_write as mg
    muc = mg._item_from_submit({"title": "Tin một", "link": "https://www.reuters.com/x"},
                         1, [], "nova", "nova")
    assert muc["via"], "khong suy duoc via"
    muc2 = mg._item_from_submit({"title": "Tin", "link": "https://a.vn/1", "via": "tay-viet"},
                          1, [], "nova", "nova")
...
def test_muc_tu_nop_don_em_dash_giong_nhanh_Finn():
    ...
    import manifest_write as mg
    muc = mg._item_from_submit({"title": "Tin", "link": "https://a.vn/1",
...
def test_muc_tu_nop_category_mac_dinh_theo_vai():
    import manifest_write as mg
    n = mg._item_from_submit({"title": "T", "link": "https://a.vn/1"}, 1, [], "nova", "nova")
    v = mg._item_from_submit({"title": "T", "link": "https://a.vn/1"}, 1, [], "vera", "vera")
