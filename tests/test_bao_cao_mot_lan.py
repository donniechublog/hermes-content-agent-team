...
Chay:  venv/bin/python tests/test_bao_cao_mot_lan.py
"""
import json
import os
import sys
...
def test_quet_nop_khong_gui_ban_mat_tin():
...
def test_quet_nop_khong_gui_ban_tieu_de_mat_dau():
    """Lan chay THU HAI hom 12/09 — chinh ban Ong Chu reply vao."""
    with tempfile.TemporaryDirectory() as t:
        ma, da_gui = _chay_main(t, stderr=CANH_MAT_DAU)
    assert da_gui == [] and ma == 1, (ma, da_gui)


def test_quet_nop_van_gui_khi_chi_co_canh_bao_nhe():
    with tempfile.TemporaryDirectory() as t:
        man = Path(t) / "vera_candidates_2026-09-12.json"
        man.write_text("{}", encoding="utf-8")
        ma, da_gui = _chay_main(t, stderr=CANH_TU_THEM, manifest=man)
...
def _gui_gia(handler, text, luu_mid):
...
def test_luu_mid_nho_moi_manh_cua_bao_cao_dai():
    ...
    with tempfile.TemporaryDirectory() as t:
        ...
        d = _gui_gia(handler, dai, Path(t) / "bao_cao_mid.vera.json")
...
def test_reply_vao_manh_dau_van_la_lenh_chon():
    with tempfile.TemporaryDirectory() as t:
...
def test_reply_vao_bao_cao_cu_van_bi_tu_choi():
...
def test_go_troi_khong_phai_lenh():
    ...
    with tempfile.TemporaryDirectory() as t:
        dct = _dat_mid(t, message_id=2005, message_ids=[2005])
...
def test_duong_ra_moi_khong_de_len_ban_chay_cung_phut():
