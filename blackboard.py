...
def _standard_home(draft_id: str) -> None:
...
def create_root(draft_id: str, title: str, goal: str, author: str) -> tuple:
    ...
    import schema
    _write_meta(draft_id, schema.merge_meta(_meta(draft_id), {"root_task": rid}))
    return rid, True


def write(draft_id: str, key: str, value, author: str) -> bool:
...
def write_background(draft_id: str, key: str, value, author: str = "script", hermes_home=None) -> tuple:
...
def main() -> int:
    ...
    _standard_home(a.draft_id)
    if a.lenh == "root-of":
        print(root_of(a.draft_id))
        return 0
    if a.lenh == "doc":
        print(json.dumps(read(a.draft_id), ensure_ascii=False, indent=2))
        return 0
    if a.lenh == "root":
        rid, moi = create_root(a.draft_id, a.title, a.goal, a.author)
        print(f"[bang-den] thẻ gốc {'mới' if moi else 'đã có'}: {rid}", file=sys.stderr)
        print(rid)                       # dòng cuối stdout = id, để script khác đọc
        return 0
    try:                                 # ghi: best-effort
        ok = write(a.draft_id, a.key, _value(a.value), a.author)
