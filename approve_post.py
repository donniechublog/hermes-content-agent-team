#!/usr/bin/env python3
"""approve_post.py — NUT BAM tren ban nhap va tren album anh: Duyet/Bo/Lam lai (hoi ly
...
from approve_dispatch import (  # noqa: E402
    ...
    standard_assignee, kanban_create, link_result, reason_task,
)
...
def _write_mark(p_draft, d, khoa, mid):
    ...
    try:
        d[khoa] = mid or True
        _write_json(p_draft, d)
    except Exception as e:                                   # noqa: BLE001
        print(f"[publish] khong ghi duoc {khoa}: {e}", file=sys.stderr)


def _text_one_attempt(token, channel, caption, p_draft, d, draft_id):
...
def _hand_redo(draft_id, slide=None, ly_do=None):
    ...
    if slide == "CA BO":
        ...
        _write_forbid_image_redo(draft_id, [int(x) for x in re.findall(r"\d+", slide)])
...
def _wait_within_topic(cho: dict, thread_id) -> list:
...
def _label_reason_redo(token, group, msg, thread_id, text):
    ...
    with _KHOA_LAM_LAI:
        cho = _load_redo_wait()
        ds = _wait_within_topic(cho, thread_id)
        if not ds:
            return False
    ...
    _run_background("lamlai", _process_reason_redo, token, group, thread_id,
              token, group, msg, thread_id, ho_so["draft_id"], text)
    return True

def _process_reason_redo(token, group, msg, thread_id, draft_id, text):
    with _lock_of(draft_id):
        t = text.strip().lower()
        if t in ("hủy", "huy", "bỏ", "bo", "thôi", "thoi", "cancel"):
            note = "↩️ Đã huỷ làm lại — giữ nguyên ảnh hiện tại"
        elif t in ("làm lại", "lam lai", "redo", ""):
            note, _ = _hand_redo(draft_id)              # khong neu ly do -> kieu cu
        else:
            slide, ly_do = _extract_reason_redo(text)
            note, _ = _hand_redo(draft_id, slide, ly_do)
...
def _redo_all_done_limit(token, group):
    ...
    for ho_so in het:
        ...
        _run_background("lamlai-han", _hand_all_done_limit, token, group, tid,
                  token, group, tid, ho_so["draft_id"])

def _hand_all_done_limit(token, group, thread_id, draft_id):
    with _lock_of(draft_id):
        note, _ = _hand_redo(draft_id)
...
def create_task_kite(draft_id: str, im: dict, ly_do: str = "") -> tuple:
    ...
    import task_bodies
    body_cu = im.get("body", "")
    link = im.get("link") or _extract_line(body_cu, "Link")
    summary = im.get("summary") or _extract_line(body_cu, "Tom tat")
    source_note = im.get("source_note") or _extract_line(body_cu, "Nguon")
...
def _button_drop_limit(token, draft_id, cq, wp):
    ...
    if w.get("created") is True:
        ...
        call(token, "answerCallbackQuery", callback_query_id=cq["id"],
             text="Đã bỏ hẳn")
        _go_count_image(draft_id, "bo han tin")
        if wp.exists():
            try:
                w["created"] = "rejected"
                _write_json(wp, w)
...
def _button_kite(token, chat_id, draft_id, cq):
...
def _button_redo(token, chat_id, draft_id, cq, msg):
    ...
    if not ip.exists():
        ...
        with _KHOA_LAM_LAI:
            cho = _load_redo_wait()
            khac = [v for v in _wait_within_topic(cho, thread_id)
                    if v.get("draft_id") != draft_id and not _over_limit(v)]
            if not khac:
        ...
        _go_count_image(draft_id, "lam lai album")
        n = int(im.get("remakes", 0)) + 1
        if im.get("carousel"):
...
def _button_approve(token, chat_id, draft_id, cq, wp):
    ...
    if not wp.exists():
        ...
        if w.get("created") is True:
            ...
            # nam todo mai.
            _tt_cha = {t: _status_task(t) for t in (w.get("root_task"), w.get("dre_task")) if t}
...
def handle_img_approval(token, action, draft_id, cq):
    ...
    keyboard = None
    if action == "imgno":
        note = _button_drop_limit(token, draft_id, cq, wp)
    elif action == "imgkite":
        note = _button_kite(token, chat_id, draft_id, cq)
    elif action == "imgtiep":
        note, keyboard = _button_lower_ready(token, draft_id, cq)
    elif action == "imgredo":
        note = _button_redo(token, chat_id, draft_id, cq, msg)
        if note is None:
            return
    else:                                                       # imgok
        note = _button_approve(token, chat_id, draft_id, cq, wp)
    _finalize_button(token, msg, draft_id, note, keyboard)
...
def handle_callback(token, channel, cq):
    ...
    if action in ("mlai", "mlaif", "mlaii", "mlait"):
        _bottom_again_moat(token, action, draft_id, cq)
        return
...
BACKGROUND_LAYER_BUTTON = {"mlaif": ("facebook_post", "Facebook"),
                "mlaii": ("instagram_carousel", "Instagram"),
                "mlait": ("tiktok_slide", "TikTok")}


def _bottom_again_moat(token, action, draft_id, cq):
