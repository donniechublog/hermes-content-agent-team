#!/usr/bin/env python3
"""gin_chuan_bi.py — BRIEF cho Gin (thay chữ Anh bằng chữ Việt trên THẺ QUOTE):
...
chỉ viết bản dịch tiếng Việt rồi chạy gin_nop.py.
...
Dùng:
    ...
    venv/bin/python gin_chuan_bi.py "https://www.instagram.com/p/ABC/?img_index=7"
    venv/bin/python gin_chuan_bi.py "https://www.instagram.com/p/ABC/" --slide 7
"""
import argparse
import json
import re
...
def _tai_link(url: str, slide: int = None) -> tuple:
...
def mau_chu(img_bgr, box) -> list:
    ...
    crop, chu = _tach_chu(img_bgr, box)
    if crop is None:
        return [255, 255, 255]
    if chu is None:
...
def dac_trung_chu(img_bgr, box, text: str) -> dict:
    ...
    font = ve_chu.chon_font(_BUT, text, adv, muc >= DAM_MUC)
    return {"cao_net": cao, "muc": round(muc, 3), "adv": round(adv, 3), "font": font}


def do_nen(img_bgr, boxes: list, i: int) -> dict:
    ...
    med = np.median(px, axis=0)
    return {"nen": "phang" if std <= PHANG_STD else "anh", "std_nen": round(std, 1),
...
def ocr_vung(anh: Path) -> tuple:
    ...
    for i, (box, text, conf) in enumerate(res):
        ...
        v.update(do_nen(img, boxes, i))
        v.update(dac_trung_chu(img, box, text))
        vung.append(v)
    _doc_can_le(vung, img.shape[1])
...
def _doc_can_le(vung: list, w_anh: int) -> None:
...
def viet_brief(id_: str, anh: Path, img, vung: list, wd: Path) -> str:
    ...
    L += ["", f"## Spec — viết vào {wd}/spec.json",
          ...
          "viết PIL script, KHÔNG vision_analyze từng ảnh, KHÔNG chạy doi_chu_anh.py/"
          "gui_telegram.py tay."]
    return "\n".join(L)
