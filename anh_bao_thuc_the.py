...
TOI_DA_MOI_MIEN = 2               # mot toa soan khong lap ca kho
TOI_DA_BAI = 24
...
def loc_bai(items: list, bo_mien: tuple = (), toi_da: int = TOI_DA_BAI) -> list:
    ...
    for link, title in items:
        ...
        if not u.startswith("http") or u in thay:
            continue
        mien = _mien(u)
        if not mien or any(b in mien for b in nguon_bai.BO_MIEN + tuple(bo_mien)):
            continue
        if dem.get(mien, 0) >= TOI_DA_MOI_MIEN:
            continue
        thay.add(u)
...
def _rss(q: str, mkt: str) -> list:
    try:
        r = nguon_bai._tai(BING_RSS.format(q=up.quote(q), mkt=mkt), 20)
...
def bao_ve(tu_khoa: list, bo_mien: tuple = (), toi_da: int = TOI_DA_BAI) -> list:
    """Bài báo gần đây về các từ khoá (mỗi từ khoá × MKT). Trả [{url, tieu_de, mien}]."""
    items = []
    for q in tu_khoa:
        for mkt in MKT:
            items += _rss(q, mkt)
    return loc_bai(items, bo_mien, toi_da)


def og_tu_html(html: str, url_bai: str) -> str | None:
    for rx in _OG:
        m = rx.search(html or "")
        if m:
            u = m.group(1).strip()
...
def anh_bao_thuc_the(tu_khoa: list, bo_mien: tuple = (), so_anh: int = 12) -> list:
    """Ứng viên ảnh (og:image) từ báo chí về các từ khoá. [] khi không ra gì."""
    bai = bao_ve(tu_khoa, bo_mien)
