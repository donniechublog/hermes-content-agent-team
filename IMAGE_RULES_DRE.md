# LUẬT ẢNH CỦA DRE (`carousel`, `carousel.py`)

**LOW-182 (16/09/2026):** Ông Chủ đảo ngược quyết định 04/09/2026 ("làm một bộ
chung thay vì mỗi vai một bộ") — tách tài liệu dùng chung `IMAGE_RULES_DRE.md`
thành **ba bản riêng**, mỗi vai làm ảnh (Ethan/Dre/Kite) một bản, bắt đầu từ
đúng nội dung bản chung tại thời điểm tách. Đây là **bản của Dre** — sửa ở
đây không còn tự động áp sang bản của Ethan/Kite nữa.

Toàn bộ nội dung dưới đây là **lịch sử** dẫn tới bộ luật hiện tại của Dre
...
**Đường cắt — một câu, vẫn còn đúng cho riêng Dre:**

> *"Ảnh này có được dùng không"* → nằm ở đây (và thành cổng chặn trong
> `image_rules_dre.py`).
> *"Đặt nó lên khung thế nào"* → nằm trong `carousel.py`.
...
- **Chỗ đứng**: nhãn 🏢 ẢNH THƯƠNG HIỆU. **Khác ảnh khái niệm ở hai điểm**: nó
  ...
  (`MAX_EXTRA_BRAND_`), tối đa **4 tấm mỗi hãng** (`MAX_NEW_RANK`, LOW-263
  19/09/2026 — trước là 2, nhưng trần tổng + round-robin giữa các hãng khi cắt
  đã tự lo việc "không thành album một hãng" rồi; cap riêng ở mức 2 chỉ còn bóp
  chết đúng ca tin **một hãng duy nhất, ít nguồn** — nơi cần ảnh hãng nhất).
...
`brand_match.person` (vòng thương hiệu gắn từ Wikidata), tên riêng trong `description`
...
tên bịa lọt cổng — trong khi `description` đã nói rõ "CEO Jensen Huang". Trước đó chỉ
...
## 9. Bảng cổng chặn (`image_rules_dre.py`)
