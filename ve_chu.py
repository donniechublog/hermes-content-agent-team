#!/usr/bin/env python3
"""ve_chu.py — VE CHU VIET len anh, dung CHUNG cho Gin va Itachi.

Tach ra khoi itachi_nop.py (07/09/2026) khi Gin nhan viec thay chu tren the
quote: hai vai ve cung mot kieu, nen luat ve phai nam MOT cho. Truoc do
`_ve_khoi` chi co trong itachi_nop.py; de Gin import cua Itachi la buoc mot vai
...
CO_MIN = 16
CO_DO = 200        # co chu dung de DO ti le muc/co — du lon de bot sai so lam tron


def duong_font(font_key: str) -> str:
    return str(FONT.get(font_key) or FONT["regular"])


def font_mac_dinh(h_vung: int, h_anh: int) -> str:
...
def cao_muc(d: ImageDraw.ImageDraw, text: str, font) -> int:
    """Chieu cao MUC that cua mot dong khi ve (bbox chu, khong phai bbox font)."""
    bb = d.textbbox((0, 0), text, font=font)
    return max(1, bb[3] - bb[1])


def co_theo_goc(d: ImageDraw.ImageDraw, text_goc: str, cao_net: int, font_key: str) -> int:
    ...
    f = _f(duong_font(font_key), CO_DO)
    return max(CO_MIN, round(CO_DO * cao_net / cao_muc(d, goc, f)))


def ti_le_rong(d: ImageDraw.ImageDraw, text: str, font_key: str) -> float:
...
def chon_font(d: ImageDraw.ImageDraw, text: str, adv_goc: float, dam: bool) -> str:
    ...
    if not adv_goc or len(ung) == 1:
        return ung[0]
    return min(ung, key=lambda k: abs(ti_le_rong(d, text, k) - adv_goc))
...
def tran_hop(d: ImageDraw.ImageDraw, text: str, w: int, h: int, font_key: str,
    ...
    """
    f = _f(duong_font(font_key), CO_MIN)
    lines = _wrap(d, text, f, w)
    lh = _gian_dong(d, lines, f, _khe(buoc, cao_goc, co, CO_MIN))
    return max(0, lh * len(lines) - int(h * 1.05))


def _gian_dong(d: ImageDraw.ImageDraw, lines: list, f, khe: float = None) -> int:
    """Giãn dòng khi vẽ.
    ...
    cao = max((cao_muc(d, ln, f) for ln in lines if ln.strip()), default=1)
    return int(cao + max(0, khe))


def _khe(buoc: int, cao_goc: int, co: int, size: int):
...
def chon_co(d: ImageDraw.ImageDraw, text: str, w: int, h: int, font_key: str,
            co: int = None, buoc: int = None, cao_goc: int = None) -> int:
    ...
    path = duong_font(font_key)
    size = max(CO_MIN, int(co if co else h * 0.82))
    while size > CO_MIN:
        f = _f(path, size)
        lines = _wrap(d, text, f, w)
        if _gian_dong(d, lines, f, _khe(buoc, cao_goc, co, size)) * len(lines) <= h * 1.05:
            break
        size -= 2
    return size


def ve_khoi(d: ImageDraw.ImageDraw, text: str, x: int, y: int, w: int, h: int,
    ...
    path = duong_font(font_key)
    size = chon_co(d, text, w, h, font_key, co, buoc, cao_goc)
    f = _f(path, size)
    lines = _wrap(d, text, f, w)
    lh = _gian_dong(d, lines, f, _khe(buoc, cao_goc, co, size))
    yy = y + max(0, (h - lh * len(lines)) // 2)
    for ln in lines:
        tw = d.textlength(ln, font=f)
        xx = x + (w - tw) / 2 if align == "center" else x
        d.text((xx, yy), ln, font=f, fill=mau(color))
        yy += lh
