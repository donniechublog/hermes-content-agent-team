...
Dung:
    venv/bin/python material.py --tieu-de "..." --link "..." --out /tmp/tu_lieu.md
"""
import argparse
import json
import re
...
COUNT_ARTICLE_OTHER = 2
TEXT_MAX = 6000          # moi nguon, du de Miles doc ma khong phinh prompt

# Cau co so lieu — thu Miles thuc su can. Bat ca % , diem, gia, kich thuoc model.
HAS_COUNT = re.compile(
...
def sentence_has_count(doan: list) -> list:
...
def gather(tieu_de: str, link: str, so_bai_khac=COUNT_ARTICLE_OTHER, tu_nguon=None) -> dict:
    """tu_nguon: tep JSON do article_images.py --luu-nguon sinh ra.
    ...
    if not dsach:
        try:
            ...
            dsach = article_images.other_outlets(tieu_de, link, so=so_bai_khac * 2)[:so_bai_khac]
        except Exception as e:                               # noqa: BLE001
...
def main():
    ...
    ap.add_argument("--so-bai-khac", type=int, default=COUNT_ARTICLE_OTHER)
    ap.add_argument("--tu-nguon", help="Tep nguon do article_images.py --luu-nguon sinh ra")
    a = ap.parse_args()
