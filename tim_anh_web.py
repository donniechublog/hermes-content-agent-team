...
BO_MIEN_WEB = ("ytimg.com", "pinimg.com", "pikabu.ru", "playground.ru", "slideserve.com",
...
def _dung_duoc(u: str) -> bool:
    if not u or not u.startswith("http") or len(u) > 500:
        return False
    if any(b in u for b in BO_MIEN_WEB):
        return False
...
def loc(urls: list, so: int, tu: str, q: str) -> list:
    thay, ra = set(), []
    for u in urls:
        if u in thay or not _dung_duoc(u):
            continue
        thay.add(u)
...
def _yandex(page, q: str, so: int) -> list:
    ...
    return loc(yandex_img_url(hrefs), so, "web_yandex", q)


# Bing giu code de bat lai khi IP doi; hien tai lech de 3/5 (xem docstring).
NGUON = (("Yandex", lambda page, q, so: _yandex(page, q, so)),)
...
if __name__ == "__main__":
    for c in tim_anh_web(" ".join(sys.argv[1:]) or "TSMC fab"):
