...
MAX_FROM_SUMMARY = 15          # brief cua ca ba vai hua "mot menh de <= 15 tu"
_EM_DASH = re.compile(r"\s*[—–]\s*")


def pick_by_k(k_tho, danh_sach: list, nhan: str) -> tuple:
...
def single_summary(tom, nhan: str) -> tuple:
    ...
    if so_tu > MAX_FROM_SUMMARY:
        ...
        canh.append(f"summary_vi {so_tu} tu (> {MAX_FROM_SUMMARY}), giu nguyen "
                    f"nhung nen rut ({nhan}): {tom[:60]}")
    return tom, canh


def list_count(items: list) -> list:
...
def write_manifest(out: Path, vai: str, items: list) -> None:
...
def finalize_required(vai_bb: str, items: list, bo_qua: bool) -> None:
    """Xoa cac muc BAT BUOC da thuc su vao manifest, va in con lai bao nhieu."""
    if bo_qua:
        return
    import required
    da = required.delete(vai_bb, items)
