#!/usr/bin/env python3
"""ethan_image_rules.py — BO TIEU CHI ANH CUA RIENG ETHAN (hero-image, card.py).

Ong Chu chot 16/09/2026: "tu gio ko dung image_rules chung nua, moi designer mot
file rieng", va moi tep COPY DU tieu chi nen chu khong tro ve loi chung (LOW-183).
Day la DAO quyet dinh 04/09/2026 ("lam MOT bo tieu chi chung thay vi moi vai mot
bo") — con nguyen van tien le do trong `image_rules.py`.

Vi sao dao: su co 15/09/2026 (LOW-177/179/180) cho BA bai Ethan ra anh xep hang
khoanh sai hang, va vi cong dung chung nen ban va cho Ethan buoc phai di vao dung
cho Dre cung dung (`describe_ranking_image`).

GIA DA BIET VA DA NHAN: tep nay la mot BAN COPY cua `image_rules.py`. Ba ban
(Ethan/Dre/Kite) SE LECH NHAU theo thoi gian. Sua mot luat nen la phai sua ba cho.
Gia cua chia le da do hoi 04/09: hai phien lam hai lan cung viec "nhan dien
chart", mot ban ra ket qua sai va chan nham viec dung.

PHAM VI: chi `card.py` va `ethan_submit.py` dung tep nay. Cong Ethan THUC SU goi:
check_blank_image, check_resolution, check_unnamed_face, check_duplicate,
check_chart_standalone, check_crop_landscape. Xem bang o IMAGE_RULES_ETHAN.md §9.

CHUA TACH (buoc 4 cua LOW-183): `submit_common.py` — duong di cua
check_not_reused/record_used — VAN import `image_rules` chung, vi so
`state/<brand>/anh_da_dung.jsonl` la so dung chung giua cac vai. Tach so la sua
state production cua 2 brand nen lam rieng.

CACH DUNG: moi ham `check_*` tra ve (loi, canh_bao) — hai danh sach chuoi. Khong
