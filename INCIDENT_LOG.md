...
## Nội dung nằm ở `incident_journal/`, mỗi sự cố một tệp
...
```
incident_journal/YYYY-MM-DD-short-name.md   mỗi sự cố một tệp, viết xong không sửa lại
incident_journal/reference.md                mục không phải sự cố (model, provider, kiểu `dai`)
```
...
`ls incident_journal/` đã là mục lục xếp theo thời gian.

Đọc gộp toàn bộ theo thứ tự thời gian:

```bash
cat incident_journal/*.md
```

Tìm một sự cố cũ:

```bash
grep -rl "tu khoa" incident_journal/
```
