#!/usr/bin/env python3
"""gin_nop.py — NỘP của Gin: trám nền THẺ QUOTE rồi VẼ CHỮ VIỆT lên đúng chỗ,
...
lên): `itachi_chuan_bi.py` gọi nó, đừng bỏ.

Dùng:
    venv/bin/python gin_nop.py 338                 # spec ở state/<brand>/chuan_bi/gin_338/spec.json
...
TI_LE_CO_TOI_THIEU = 0.75   # chu ve ra duoi 75% co chu goc thi coi la khong dat
...
def _hop_dich(d: dict, spec: dict) -> tuple:
    ...
    for k, val in khai.items():
        ...
        if not isinstance(val, dict):
            ...
            continue
        text = bo_dau_cam(str(val.get("text") or "").strip())
        if not text:
...
def lam_the(id_: str, wd: Path, spec: dict, bo_qua_dau: bool) -> tuple:
    ...
    for kh in khoi:
        ...
        mong = ve_chu.co_theo_goc(do, kh["text_goc"], kh["cao_net"], kh["font"])
        tran = ve_chu.tran_hop(do, kh["text"], kh["w"], kh["h"], kh["font"],
                               mong, kh["buoc"], kh["cao_net"])
        if tran:
            loi.append(f"vùng {kh['stt']}: bản dịch {kh['text'][:36]!r} tràn hộp {tran}px kể cả "
                       f"khi đã nhỏ hết cỡ ({ve_chu.CO_MIN}px) — rút gọn câu.")
            continue
        ...
        that = ve_chu.chon_co(do, kh["text"], kh["w"], kh["h"], kh["font"], mong,
                              kh["buoc"], kh["cao_net"])
        if that < mong * TI_LE_CO_TOI_THIEU:
            loi.append(f"vùng {kh['stt']}: bản dịch {kh['text'][:36]!r} phải thu xuống {that}px "
                       f"mới vừa hộp, chữ gốc {mong}px — lệch quá {int((1-TI_LE_CO_TOI_THIEU)*100)}%, "
    ...
    for kh in khoi:
        for v in kh["vung_goc"]:
            ...
            # lai dung mau nen.
            no = max(doi_chu_anh.DILATE_PX, int((v.get("cao_net") or v["h"]) * 0.15))
            m = doi_chu_anh.dung_mask(img, [(_no_box(v["box"], img.shape), v["text"], 1.0)],
    ...
    for kh in khoi:
        co = ve_chu.co_theo_goc(dd, kh["text_goc"], kh["cao_net"], kh["font"])
        ve_chu.ve_khoi(dd, kh["text"], kh["x"], kh["y"], kh["w"], kh["h"],
...
def main() -> int:
    ...
    ap.add_argument("id", help="id đã chạy gin_chuan_bi.py")
    ...
    out, khoi, loi = lam_the(a.id, wd, spec, a.bo_qua_dau)
    if loi:
        for e in loi:
            print(f"[LOI] {e}")
        return nc.dem_vong_loi(wd, loi, f"venv/bin/python gin_nop.py {a.id}")
    ...
    if a.khong_gui:
        ...
        try:
            res = gui_telegram.post("gin", [str(out)], mo_ta[:1000], reply_to=reply)
