#!/usr/bin/env python3
"""cape_prepare.py — BRIEF cho Cape (teaser mời đọc bài donniechu.com).
...
+ các đoạn văn thuần vào spec.json rồi chạy cape_submit.py.

Dùng:
    venv/bin/python cape_prepare.py "https://www.donniechu.com/posts/..."
"""
import argparse
import json
import re
...
def extract(url: str, wd: Path, lam_moi: bool) -> dict:
    p = wd / "article.json"
...
def write_brief(url: str, d: dict, wd: Path) -> str:
    ...
    L += ["", f"## Viết spec vào: {wd}/spec.json",
          ...
          f"cd {ROOT} && venv/bin/python cape_submit.py \"{url}\"",
...
def main() -> int:
    ...
    a = ap.parse_args()
    wd = workdir(a.url)
    d = extract(a.url, wd, a.lam_moi)
    (wd / "url.txt").write_text(a.url, encoding="utf-8")
    brief = write_brief(a.url, d, wd)
