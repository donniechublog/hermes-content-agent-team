...
PREFIX = {"nova": "nova_candidates", "market": "vera_candidates",
...
def _item_from_submit(it: dict, i: int, nguon: list, vai: str, vai_bb: str) -> dict | None:
    ...
    if k is not None and nguon:
        t, loi = mc.pick_by_k(k, nguon, f"muc {i}")
        if not t:
    ...
    if not link and it.get("title"):
        ...
        for v in required.read(vai_bb).values():
            if required.match(v, {"title": it["title"], "summary_vi": ""}):
...
def extra_required(items: list, nguon: list, vai: str, vai_bb: str) -> list:
    ...
    for v in required.check(vai_bb, items):
        link = required.link_call_y(v)
        if not link.lower().startswith(("http://", "https://")):
...
def main():
    ...
    for i, it in enumerate(ds, 1):
        muc = _item_from_submit(it, i, nguon, a.vai, vai_bb)
        if muc is not None:
            items.append(muc)
    ...
    if out.exists() and not a.out:
        out = mc.path_out_new(STATE / f"{PREFIX[a.vai]}_{ngay}.json")
    mc.write_manifest(out, a.vai, items)
    print(out)
