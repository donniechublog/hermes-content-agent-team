...
cũng chạy cho donniechublog ở container blog — cùng một `carousel.py`, khác đúng
một cờ `--brand dcgr`, giống hệt vai `designer` (hero image) chạy cả hai brand.
...
2. **`--brand dcgr` là điểm khác duy nhất giữa hai brand.** Thiếu cờ này
   thì watermark ra `donniechublog` — sai thương hiệu. Người đọc dcgr.tech khác
   donniechublog: dân kinh doanh, tài chính, truyền thông (cùng gu với `writer`
   dcgr) — chọn tin và giọng slide phù hợp hướng đó.
