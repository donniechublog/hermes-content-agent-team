...
Dung:
    venv/bin/python soat_cron.py               # soat, gui neu co van de
    venv/bin/python soat_cron.py --khong-gui   # chi in ra man hinh
    venv/bin/python soat_cron.py --luon-bao    # gui du da bao roi (de thu)
"""
import argparse
import html
import json
...
DAU = env_load.ROOT / "state" / "soat_cron.json"
...
def soat_kho(cron_dir: Path, bay_gio: float) -> tuple:
    ...
    elif bay_gio - nhip > TICK_CU:
        van_de.append({"muc": "KET", "ten": "ticker",
                       "ly_do": [f"nhịp cuối {_tuoi(bay_gio - nhip)} trước "
                                 f"(ngưỡng {TICK_CU}s) — MỌI job trong kho này đang không nổ"]})
    elif thanh is not None and nhip - thanh > TICK_CU:
        # Nhip con dap nhung khong tick nao thanh cong: ticker song ma hong.
        van_de.append({"muc": "KET", "ten": "ticker",
                       "ly_do": [f"còn nhịp nhưng lần tick THÀNH CÔNG cuối đã "
                                 f"{_tuoi(bay_gio - thanh)} trước"]})
        ...
        if job.get("enabled", True) and (job.get("state") or "") != "paused" and ke:
            try:
                ...
                if tre > TRE_GIAY:
                    ...
                    ly_do.append(f"lỡ hẹn {_tuoi(tre)} (đáng lẽ chạy lúc {_gio(ke)})")
            except (TypeError, ValueError):
                muc = "HONG"
...
def soat(homes=None, bay_gio=None) -> tuple:
    ...
    for brand, home in sorted(homes.items()):
        if not home.is_dir():
            thieu.append(brand)
            continue
        for cron_dir, profile in kho_cron(home):
            v, n = soat_kho(cron_dir, bay_gio)
            tong += n
            for m in v:
...
def doc_dau() -> dict:
    try:
        return json.loads(DAU.read_text(encoding="utf-8"))
    except (OSError, ValueError):
        return {}


def dung_tin(van_de, tong, thieu, ngay: str, so_brand: int) -> str:
    ...
    for m in sorted(van_de, key=lambda x: (x["muc"] != "HONG", x["brand"], x["ten"])):
        dong.append(f"{MUC.get(m['muc'], '•')} <b>{e(m['brand'])} / {e(m['ten'])}</b>")
        for l in m["ly_do"]:
            dong.append(f"    {e(l)}")
        dong.append("")
    for b in thieu:
...
def main() -> int:
    ...
    van_de, tong, thieu = soat(homes)
    ngay = datetime.now().strftime("%d/%m")
    khoa = khoa_van_de(van_de) + [f"thieu-home/{b}" for b in sorted(thieu)]
    ...
    if khoa:
        tin = dung_tin(van_de, tong, thieu, ngay, len(homes))
