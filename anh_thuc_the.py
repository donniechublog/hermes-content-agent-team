...
CANH_NGAN_MIN = 700
TOI_DA_THUC_THE = 4          # cum ten rieng thu, tinh ca ten model
TOI_DA_MOI_THUC_THE = 2      # anh moi thuc the tu Commons


def thuc_the_trong_tieu_de(tieu_de: str, models: list | None = None) -> list:
    ...
    while i < len(ws):
        w = ws[i]
        if w[:1].isupper() and w.isalpha() and len(w) >= 3 and w.lower() not in nguon_bai.TU_RONG_TRUY_VAN:
            cum = [w]
            j = i + 1
    ...
    ra = [c for c in ra if len(c.split()) >= 2 or c.lower() in th.TEN_HIEN]
    return ra[:TOI_DA_THUC_THE]
...
def pageimages(ten: str) -> dict | None:
    ...
    if not o.get("source") or min(o.get("width", 0), o.get("height", 0)) < CANH_NGAN_MIN:
        return None
...
def commons_theo_cum(ten: str, so: int = TOI_DA_MOI_THUC_THE) -> list:
    ...
    import anh_thuong_hieu as th
    pages = th._hoi_commons(f'"{ten}"')
    if not pages:
        return []
    dt = th._tu_dac_trung(ten)
    ra = []
    for pg in pages.values():
        ...
        thap = tep.lower()
        if anh_khai_niem.TEN_LOAI.search(thap) or not th._co_cum(dt, thap):
            continue
...
def anh_thuc_the(tieu_de: str, models: list | None = None) -> list:
    ...
    ra, da = [], set()
    for ten in thuc_the_trong_tieu_de(tieu_de, models):
        for c in ([pageimages(ten)] if True else []) + commons_theo_cum(ten):
