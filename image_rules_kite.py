#!/usr/bin/env python3
"""image_rules_kite.py — BO TIEU CHI ANH RIENG CUA KITE (`carousel-edu`, `render_edu.py`).
...
`image_rules_dre.py` nua — muon dong bo thi phai tu tay sua ca ba.
...
DUONG CAT cu van con dung — mot cau:
    "Anh nay co DUOC DUNG khong"      -> nam o day (rieng cua Kite).
    "Dat no LEN KHUNG the nao"        -> rieng vai, o lai renderer (`render_edu.py`).

CACH DUNG: moi ham `kiem_*`/`check_*` tra ve (loi, canh_bao) — hai danh sach
chuoi. `render_edu.py`/`kite_submit.py` tu chon cong nao hop voi khung cua
...
# Chu the (anh CHUP) duoc lan xuong duoi dinh khoi chu #figtxt bao nhieu phan khung
# (dung sai hop mat/hop vision) — LOW-273. Bang/bieu do khong xet: anh chuan 19/09 (bang
# DeepSeek, slide 07) chay xuong duoi chu va Ong Chu coi la chuan.
...
def check_aspect_ratio(nhan, p, w, h, lo=TI_LE_45, hi=TI_LE_11, dung_sai=TOLERANCE_RATIO, img=None):
    ...
    if r >= LANDSCAPE_CLEAR:
        ...
        return [f"{nhan}: ti le {w}x{h} ({r:.2f}) khong nam trong 4:5..1:1. Anh "
                ...
                f"duoc cat be ngang: crop_ratio.py --anh {p} --ra <ra.png> --vai kite "
                f"--ti-le 4:5 --cat-ngang"], []
    return [f"{nhan}: ti le {w}x{h} ({r:.2f}) khong nam trong 4:5..1:1 — cat "
            f"truoc: venv/bin/python crop_ratio.py --anh {p} --ra <ra.png> --vai kite "
            f"[--ti-le 4:5] [--cx/--cy]"], []
