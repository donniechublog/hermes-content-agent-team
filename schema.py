...
def _so_cap_ghep_that(ds: list) -> int:
    ...
    import luat_anh
    n = len(ds)
    ke = [[j for j in range(n) if j != i and luat_anh.ghep_vua_khung(ds[i].get("ti_le"),
                                                                     ds[j].get("ti_le"))]
          for i in range(n)]
...
def so_anh_dung_duoc(anh: list) -> int:
    ...
    return (len(rieng) - len(chi_ghep)) + _so_cap_ghep_that(chi_ghep) + min(1, len(khai_niem))
