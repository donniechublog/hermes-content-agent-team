...
VIEN = "#0a0a0a"
MAU_HANDLE = "#6e6e6e"
MAU_PROMPT = "#008299"     # brand light-variant, doc duoc tren nen kem
DOTS = ("#ff5f57", "#febc2e", "#28c840")     # den giao thong macOS
K_BO_TRON_THE = 0.016
K_BO_TRON_NGOAI = 0.014
K_BONG = 0.028
...
def avatar_cho_emoji(emoji: str) -> Path | None:
...
def dong_khung(nguon, ra, emoji: str = "", handle: str = "@donniechublog",
    ...
    bo_tron = max(8, R(ngan * K_BO_TRON_THE))
    bo_tron_ngoai = max(6, R(ngan * K_BO_TRON_NGOAI))
    ...
    N = 4
    lop = Image.new("RGB", (CW * N, CH * N), _mau(BG))
    ...
    d.rounded_rectangle([bong * N, bong * N, (bong + the_w) * N - 1, (bong + the_h) * N - 1],
                        radius=bo_tron_ngoai * N, fill=_mau(VIEN))
    ...
    d.rounded_rectangle([0, 0, the_w * N - 1, the_h * N - 1], radius=bo_tron_ngoai * N,
                        fill=_mau(BG), outline=_mau(VIEN), width=day * N)
    ...
    for i, mau in enumerate(DOTS):
        ...
        d.ellipse([(cx - cham_r) * N, (cham_cy - cham_r) * N,
                  ...
                  fill=_mau(mau), outline=_mau(VIEN), width=max(1, R(day / 2)) * N)
    ...
    _chu_gian(d, (the_w - canh) * N, day_header * N, handle,
              _font(FONT_DIR / "JetBrainsMono-Bold.ttf", co_header * N),
              _mau(MAU_HANDLE), gian=0.3 * N, ve_phai=True)
    ...
    if not prompt and footer is None:
        print(f"[khung] khong co dong footer cho {handle} — dung khung khong co no. "
              f"Them vao FOOTER trong khung_anh.py, hoac truyen --footer \"<dong>\".",
              file=sys.stderr)
    if prompt:
        ...
        d.text((canh * N, day_footer * N), prompt,
               ...
               fill=_mau(MAU_PROMPT), anchor="ls")
    ...
    da_chon = None
    if not khong_mascot:
        p_av = Path(avatar) if avatar else (avatar_cho_emoji(emoji) if emoji else None)
        if p_av and Path(p_av).exists():
            da_chon = Path(p_av)
            av = Image.open(p_av).convert("RGBA")
            cao = R(ngan * 0.16)
            av = av.resize((_lam_tron(av.width * cao / av.height), cao), Image.LANCZOS)
...
def _chu_gian(d, x, y, chu, font, mau, gian=0.0, ve_phai=False):
...
def main() -> int:
    ...
    if not Path(a.image).exists():
        ...
        return 1
    mo_ta = dong_khung(a.image, a.out, emoji=a.emoji, handle=a.handle, footer=a.footer,
