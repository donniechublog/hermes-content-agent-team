...
def write_brief(m: dict, da_dung: dict | None) -> str:
    ...
    L.append("Luật điền: mỗi slide MỘT ảnh, MỘT ý; `text` HOẶC `quote`+`attrib`; mỗi mã ảnh dùng đúng "
             "một lần; chart chỉ ở slide thân (script tự dán full bề ngang); ƯU TIÊN ảnh đã VỪA khung 4:5 có chủ thể, dùng "
             "MỘT ảnh — `stack` hai ảnh ngang CHỈ khi hết ảnh vừa 4:5 (cổng chặn bắt đổi nếu còn); ảnh NGANG phải `stack` "
