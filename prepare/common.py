...
# chuan_bi/drafts (rong) thay vi drafts/ that. kite_submit.py bao "Khong thay
...
MAX_IMAGE = 8              # anh giu lai sau khi loc — du cho 10 slide ke ca ghep
...
def _hdr(url: str) -> dict:
    ...
    if _WIKI.search(_domain(url)):
        return {**HDR, "User-Agent": env_load.UA_WIKI}
    return HDR


# ---- tien ich -------------------------------------------------------------
def _brand_of(meta: dict) -> str:
...
def _read_json(p: Path, mac_dinh=None):
    try:
...
def _original_domain(h: str) -> str:
