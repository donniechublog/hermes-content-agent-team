#!/usr/bin/env python3
"""check_hermes.py — kiem cac cho content-team LE THUOC NOI BO cua hermes-agent.
...
CHAY SAU MOI `hermes update`:
    ...
    venv/bin/python check_hermes.py            # CHI DOC — an toan, khong tao gi
    venv/bin/python check_hermes.py --day-du   # them mot luot chat + mot task that
...
HAS_CHAT = ["-c", "--create-if-missing", "--no-restore-cwd", "-Q", "-q"]
...
def _check_board(ten: str, db: Path, can: dict) -> list:
...
def check_column() -> list:
    ...
    for ten, db in _home_kanban():
        loi += _check_board(ten, db, COLUMN_CAN)
        ...
        for sdb in sorted(db.parent.glob("profiles/*/state.db"))[:1]:
            loi += _check_board(f"{ten}/{sdb.parent.name}", sdb, COLUMN_CAN_STATE)
    if not _home_kanban():
...
def check_has_chat() -> list:
    ...
    return [f"co `{c}` khong con trong `hermes chat --help`" for c in HAS_CHAT
